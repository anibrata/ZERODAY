...
class EnsembleClassifier:
    ...
    def score(self, X, y, use_f1_score=True):
...
class QBoostClassifier(EnsembleClassifier):
    ...
    def __init__(self, X, y, lam, weak_clf_scale=None, drop_unused=True):
        ...
        # These piece of code is taken from sklearn library
        cls, y2 = np.unique(y, return_inverse=True)
...
def qboost_lambda_sweep(X, y, lambda_vals, val_fraction=0.4, verbose=False, **kwargs):
    ...
    if verbose:
        print('{} {:7} {} {}:'.format('QBoost instance', 'lambda', 'n_features', 'score'))

    for lam in lambda_vals:
        qb = QBoostClassifier(X_train, y_train, lam, **kwargs)
        score = qb.score(X_val, y_val, use_f1_score=False)

        if verbose:
            features = qb.get_selected_features()
            print(qb.get_selected_features())
            print('{} {:<7.4f} {:<10} {:<6.3f}'.format(qb, lam, len(features), score))
        if score > best_score:
            best_score = score
            best_clf = qb
    ...
    return best_clf, best_lambda, best_features
